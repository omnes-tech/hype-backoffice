...
import { useCampaignInfluencerProfile } from "@/hooks/use-campaign-influencer-profile";
import { getUploadUrl } from "@/lib/utils/api";

export const Route = createFileRoute("/(private)/(app)/campaigns/$campaignId/influencer/$influencerId")({
...
function InfluencerProfileScreen() {
  ...
  const { campaignId, influencerId } = useParams({
    from: "/(private)/(app)/campaigns/$campaignId/influencer/$influencerId",
  });
  const { data, isLoading, isError, error } = useCampaignInfluencerProfile(
    campaignId ?? "",
    influencerId ?? ""
  );
  ...
  const handleBack = () => {
    navigate({ to: "/campaigns/$campaignId", params: { campaignId: campaignId ?? "" } });
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[40vh] gap-4">
        <p className="text-neutral-600">
          {is404 ? "Influenciador ou campanha não encontrados." : "Erro ao carregar perfil."}
        </p>
  ...
  return (
    <div className="flex flex-col gap-11 max-w-[1151px] mx-auto px-6">
      {/* Breadcrumb + título + botão (gap 24px entre breadcrumb e linha do título) */}
      <div className="flex flex-col gap-6 w-full">
        <div className="flex items-center gap-1 text-sm text-neutral-500">
          ...
          <button type="button" onClick={handleBack} className="hover:text-neutral-700">
      ...
      {/* Hero: avatar, nome, @, local, Hypeapp (design: rounded-[12px] px-4 py-5) */}
      <div className="bg-white rounded-xl px-4 py-5 flex flex-wrap items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-base font-medium text-neutral-500">@{influencer.username}</p>
          </div>
        </div>
      ...
      {/* Card único: Nicho + Sub-nicho (colunas com borda) + Sobre o influenciador */}
      ...
      {/* Métricas: título + segment (Instagram ativo) + cards */}
      <div className="bg-white rounded-xl p-5 border border-neutral-200 flex flex-col gap-6">
        ...
        {/* Linha 1: Divisão por gênero + Seguidores, Curtidas, Alcance Médio, Engajamento */}
        <div className="flex flex-wrap gap-4">
            ...
            <div className="flex flex-wrap gap-6 justify-center">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-medium text-neutral-950">
                  Mulheres ({metrics?.gender_split?.women_percent ?? 40}%)
                </span>
              </div>
              <div className="flex items-center gap-2">
                <div className="size-6 rounded bg-neutral-400" />
                <span className="text-base font-medium text-neutral-950">
                  Homens ({metrics?.gender_split?.men_percent ?? 60}%)
                </span>
              </div>
            </div>
          </div>
          <div className="flex flex-wrap gap-4 flex-1 min-w-0">
            {[
              ...
              <div
                key={item.label}
                className="bg-neutral-100 rounded-lg p-4 flex flex-col justify-between min-w-[200px] flex-1 h-[141px]"
              >
                <div className="flex items-center gap-2">
                  <div className={`size-9 rounded-lg flex items-center justify-center ${item.iconBg}`}>
                    <Icon name={item.icon as "Heart"} size={20} color="#525252" />
                  </div>
        ...
        {/* Linha 2: Publicações totais, Campanhas participadas */}
        <div className="flex flex-wrap gap-5">
          <div className="bg-neutral-100 rounded-lg p-4 flex flex-col gap-4 min-w-[256px] flex-1">
            <div className="flex items-center gap-2">
              <div className="size-9 rounded-lg bg-amber-100 flex items-center justify-center">
                <Icon name="LayoutGrid" size={20} color="#525252" />
              </div>
              <span className="text-base font-medium text-neutral-950">
                Publicações totais dentro do Hype app
              </span>
            </div>
            <p className="text-2xl font-bold text-neutral-950">{totalPosts}</p>
          </div>
          <div className="bg-neutral-100 rounded-lg p-4 flex flex-col gap-4 min-w-[256px] flex-1">
            <div className="flex items-center gap-2">
              <div className="size-9 rounded-lg bg-primary-100 flex items-center justify-center">
                <Icon name="Megaphone" size={20} color="#525252" />
              </div>
              <span className="text-base font-medium text-neutral-950">
                Campanhas participadas no Hype app
              </span>
            </div>
            <p className="text-2xl font-bold text-neutral-950">{campaignsParticipated}</p>
          </div>
        </div>
      </div>

      {/* Selo de Segurança Hypeapp */}
