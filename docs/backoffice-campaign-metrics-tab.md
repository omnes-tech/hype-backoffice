...
A documentação interativa de **toda** a API (incluindo estas rotas) está em **`/api/docs`** após subir o servidor (ex.: `http://localhost:3000/api/docs`).
...
## Integração no backoffice (este repositório)

| Endpoint | Serviço (`src/shared/services/metrics.ts`) | Hook (`src/hooks/use-campaign-metrics.ts`) | Uso na UI |
|----------|--------------------------------------------|---------------------------------------------|-----------|
| `GET …/metrics/contents` | `getCampaignContentsMetricsMap` | `useCampaignContentsMetricsMap` | Mapa `by_content_id` → totais, gráfico de engajamento por dia, Top 4, cards e modal de conteúdo |
| `GET …/metrics/top-cities` | `getCampaignTopCities` | `useCampaignTopCities` | Seção “Cidades com melhor performance” |
| `GET …/metrics/audience-by-age` | `getCampaignAudienceByAge` | `useCampaignAudienceByAge` | Gráfico de barras faixa etária + textos “Maior público Instagram/YouTube” |
| `GET …/metrics/contents/:contentId` | `getContentMetrics` (URL atualizada) | `useContentMetrics` | Reuso em outros fluxos; na tab os dados vêm do mapa em lote |

A tab `MetricsTab` consome os três primeiros hooks com o `campaignId` da rota. Se `audience-by-age` não retornar faixas, o gráfico de idade cai no **fallback ilustrativo** (mesmos números de demo de antes). Invalidação após “Verificar publicações” usa o prefixo de query `["campaigns", campaignId, "metrics"]`, que cobre essas queries.
